...
class RecordingViewController: UIViewController {
    ...
    private func showPlayerViewController(url: URL) {
        ...
        guard let offlineVC = storyboard?.instantiateViewController(identifier: "OfflineMattingViewController") as? OfflineMattingViewController else {
            return
        }
        
        navigationController?.pushViewController(offlineVC, animated: true)
        offlineVC.videoAsset = AVURLAsset(url: url)
    }
}
...
extension RecordingViewController: MovieRecorderDelegate {
