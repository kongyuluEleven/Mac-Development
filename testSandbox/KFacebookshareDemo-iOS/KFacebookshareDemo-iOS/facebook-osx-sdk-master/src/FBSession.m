...
static NSString *const FBDialogBaseURL = @"https://" FB_BASE_URL @"/dialog/";
...
@interface FBSession () <FBLoginWindowDelegate> {
    @private
...
@property(readwrite, retain)    FBLoginWindow *loginWindow;
...
- (void)handleDidBecomeActive{
    ...
    if (self.state == FBSessionStateCreated ||
        self.state == FBSessionStateClosed ||
        self.state == FBSessionStateClosedLoginFailed){
        return;
    }
...
- (void)authorizeWithPermissions:(NSArray*)permissions
    ...
    if (!didAuthNWithSystemAccount) {
      if (tryFallback) {
        // open Cobook's Facebook login dialog in
        self.loginWindow = [[[FBLoginWindow alloc] initWithWindow:nil
                                                             URL:loginDialogURL
                                                     loginParams:params
...
- (void)fbWindowLogin:(NSString*)accessToken expirationDate:(NSDate*)expirationDate
{
